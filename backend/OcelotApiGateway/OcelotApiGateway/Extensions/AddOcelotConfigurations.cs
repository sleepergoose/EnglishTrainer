...
namespace OcelotApiGateway.Extensions
{
    public static class AddOcelotConfigurations
    {
        public static IConfigurationBuilder AddOcelotConfiguration(this IConfigurationBuilder builder, string folder)
        {
            ...
            {
                throw new Exception($"Folder {folder} contains invalid configuration files");
            }
            ...
            var globalFile = configurationFiles.FirstOrDefault(file => file.Name.Equals("global.json", StringComparison.OrdinalIgnoreCase));
